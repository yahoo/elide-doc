...
* For large response bodies, Elide supports a streaming abstraction called TableExport. It works similarly to the AsyncQuery model but with three important differences:
   ...
   * Upon completion, the TableExport model includes a separate URL attribute that references the query results for streaming downloads.
...
| enableExport                      | Enable the TableExport feature.                                   | false   |
...
Long running queries may not return with the `result` populated in the responses above. The client can poll the AsyncQuery and TableExport objects asynchronously for status updates.
...
The successful completion of a TableExport request will return a URL to download the results as shown in the responses [here](/pages/guide/v{{ page.version }}/11.5-asyncapi.html#retrieving-status-and-result). Opening the URL in any browser of your choice will start downloading the results to your machine.
